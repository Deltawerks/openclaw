...
describe("native streamed lifecycle", () => {
  it.each(["thinking", "tool arguments", "async tool arguments", "async ordinary arguments"])(
    "protects credential continuity across completed %s and later text",
    async (kind) => {
      const secret = "synthetic-cross-block-secret";
      const prefix = "synthetic-cross-";
      const ordinary = kind === "async ordinary arguments";
      const suffix = ordinary ? "ordinary progress" : "block-secret";
      const execute = vi.fn(async () => ({ content: [], details: {} }));
      const response = createAssistantMessageEventStream();
      const tool = call("split", { nested: [prefix] }, kind.startsWith("async"));
      const thinking = assistant(
        kind === "thinking" ? [{ type: "thinking", thinking: prefix }] : [tool],
      );
      const text = assistant([...thinking.content, { type: "text", text: suffix }]);
      response.push({ type: "start", partial: assistant([]) });
      if (kind === "thinking") {
        response.push({ type: "thinking_start", contentIndex: 0, partial: thinking });
        response.push({
          type: "thinking_delta",
          contentIndex: 0,
          delta: prefix,
        ...
        response.push({ type: "toolcall_start", contentIndex: 0, partial: thinking });
        response.push({ type: "toolcall_end", contentIndex: 0, toolCall: tool, partial: thinking });
      }
      response.push({ type: "text_start", contentIndex: 1, partial: text });
      response.push({ type: "text_delta", contentIndex: 1, delta: suffix, partial: text });
      response.push({ type: "text_end", contentIndex: 1, content: suffix, partial: text });
        ...
        [
          {
            ...
            label: "Probe",
            description: "Probe",
            parameters: Type.Object({ nested: Type.Array(Type.String()) }),
            execute,
          },
        ],
        undefined,
        (value) => typeof value === "string" && value.endsWith(prefix),
        (value) => {
      ...
      await agent.prompt("stream").catch(() => {});
          ...
          event.type === "message_end" && event.message.role === "assistant"
            ? event.message.content.map((block) =>
                block.type === "text"
                  ...
                  : block.type === "thinking"
                    ? block.thinking
                    : Array.isArray(block.arguments.nested)
                      ? block.arguments.nested.join("")
                      : "",
              )
            : [],
        )
      ...
      if (ordinary) {
        expect(delivered).toContain(prefix + suffix);
