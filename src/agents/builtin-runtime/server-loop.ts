...
function generatedStringValues(value: unknown): string {
  if (typeof value === "string") {
    return value;
  }
  if (value !== null && typeof value === "object") {
    return Object.values(value).map(generatedStringValues).join("");
  }
  return "";
}
...
export async function runBuiltinRuntimeLoop(
  ...
  const emit = async (event: AgentEvent) => {
    ...
    native.assertProtocolSafe(payload);
    // Generated text and argument values can continue credentials across block boundaries.
    // A completed block must not release its prefix before the next block resolves it.
    const generatedText =
      (event.type === "message_start" ||
        event.type === "message_update" ||
        event.type === "message_end") &&
      event.message.role === "assistant"
        ? completedGeneratedText +
          event.message.content
            .map((block) =>
              block.type === "text"
                ...
                : block.type === "thinking"
                  ? block.thinking
                  : generatedStringValues(block.arguments),
            )
            .join("")
        : undefined;
    if (generatedText !== undefined) {
      native.assertProtocolSafe(generatedText);
    }
    ...
    if (
      ...
      event.message.role === "assistant" &&
      (native.hasCredentialPrefix(generatedText) ||
    ...
    if (event.type === "message_end" && event.message.role === "assistant") {
      ...
      // Async tool commits split one provider response into multiple assistant messages.
      // Retain generated content so the next fragment cannot complete a leaked literal.
      completedGeneratedText = generatedText ?? completedGeneratedText;
