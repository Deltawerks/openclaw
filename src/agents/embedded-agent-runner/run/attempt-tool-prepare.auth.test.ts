...
vi.mock("../../agent-tools.js", () => ({
  createOpenClawCodingToolsInternal: vi.fn(() => []),
...
describe("attempt tool credential ownership", () => {
  it("does not classify Gateway inference auth and leaves lazy tool auth available in dedicated mode", async () => {
    ...
    expect(createOpenClawCodingToolsInternal).toHaveBeenCalledOnce();
    expect(createOpenClawCodingToolsInternal).toHaveBeenLastCalledWith(
    ...
    expect(createOpenClawCodingToolsInternal).toHaveBeenCalledTimes(2);
    expect(resolveModelAuthMode).not.toHaveBeenCalled();
    expect(
      vi.mocked(createOpenClawCodingToolsInternal).mock.lastCall?.[0]?.authProfileStore,
    ).toBeUndefined();
  });
  ...
  it.each([
    ...
    expect(vi.mocked(createOpenClawCodingToolsInternal).mock.lastCall?.[0]?.authProfileStore).toBe(
      inferenceStore,
    );
  });
  ...
  it("preserves the existing tool credential owner when the inference snapshot is empty", async () => {
    await withOpenClawTestState({ label: "dedicated-tool-credential" }, async (state) => {
      ...
      const options = vi.mocked(createOpenClawCodingToolsInternal).mock.lastCall?.[0];
