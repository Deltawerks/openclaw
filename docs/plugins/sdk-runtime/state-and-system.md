...
<AccordionGroup>
  ...
  <Accordion title="api.runtime.state">
    ...
    Blob mutations use the shared SQLite worker and keep quota checks and changes in one transaction. `lookup` and `entries` use the retained read-only worker path: missing stores stay absent, and an existing selected snapshot remains selected through completion. Await all methods before publishing dependent artifacts or removing their storage. Worker errors preserve `PluginBlobStoreError` classification, operation, path, and causal errors. Byte copying, metadata serialization, and complete result materialization still use caller memory; this is not a streaming BLOB API.
